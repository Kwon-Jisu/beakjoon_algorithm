...
def train_model(start_date, **kwargs) -> str:
    ...
    if os.path.exists(model_path):
        ...
        existing_score = existing_model.score(X_test, y_test)
        print(f"- existing model score: {existing_score}")
    else:
        existing_score = -1  # If no model exists

    if score > existing_score:
        timestamp = datetime.now().strftime("%H")
        model_file = os.path.join(OUTPUT_DIR, "versions", f"model_{start_date}{timestamp}.joblib")
...
with DAG(
    ...
    train_model_task >> update_model_task >> slack_notify_task >> end
    train_model_task >> end